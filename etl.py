...
def process_song_data(spark, input_data, output_data):
    """
    Load data from song_data in s3 bucket and extract columns
    for songs and artist tables, then write the data into parquet
    files which will be loaded on s3.
    Parameters
    ----------
    spark: session
          This created spark session
    input_data: path
           This is the path to the song_data s3 bucket.
    output_data: path
            This is the path to where the parquet files will be written on s3.
    """
